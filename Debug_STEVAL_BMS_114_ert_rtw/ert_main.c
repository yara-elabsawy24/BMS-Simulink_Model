...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(Debug_STEVAL_BMS_114_M, 0);
  ...
  Debug_STEVAL_BMS_114_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Debug_STEVAL_BMS_114_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Debug_STEVAL_BMS_114_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Debug_STEVAL_BMS_114_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
  ...
  /* Terminate model */
  Debug_STEVAL_BMS_114_terminate();
